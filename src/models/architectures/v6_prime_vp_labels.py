"""FROZEN: TemporalEnrichedV6PrimePrime — v6 architecture for VP-derived labels.

DO NOT MODIFY THIS FILE once results are committed.

── Provenance ──────────────────────────────────────────────────────────────
Architecture IDENTICAL to TemporalEnrichedV6Prime. This is a separate frozen
copy to track experiment lineage for the v6-prime configuration:
  - Same architecture (single-path temporal with enriched day tokens)
  - NEW labels: VP-derived TP/SL (per-sample adaptive, aimed at VP peaks)
  - NEW regularization: dropout=0.3, weight_decay=1e-3, label_smoothing=0.1
  - NEW optimizer: AdamW (proper weight decay handling)

Why a separate file: if v6 needs bug fixes in the future, v6-prime
should not be affected. The experiment that uses these labels needs
to be reproducible with the exact architecture state at time of run.

── Architecture ────────────────────────────────────────────────────────────
(Identical to v6) Single-path temporal Transformer with enriched day tokens.

Stage 1 (spatial): For each of 30 days, sample the end-of-day VP,
    run self-attention across 50 bins with CLS pooling.
    Output: (batch, 30, 32).

Enrichment: For each day, concat:
    - VP spatial CLS output (32)
    - Daily candle features (8)
    - Daily VP structure features (8)
...
── Default dropout ─────────────────────────────────────────────────────────
Default dropout raised from 0.15 (v6) to 0.3 for v6-prime regularization.
"""
...
class TemporalEnrichedV6Prime(nn.Module):
