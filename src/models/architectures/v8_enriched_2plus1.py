"""FROZEN: EnrichedTemporalV8 — 2 spatial + 1 temporal with day token enrichment.

DO NOT MODIFY THIS FILE once results are committed.

── Provenance ──────────────────────────────────────────────────────────────
Extension of v6_temporal_enriched (1+1, 24,737 params) to 2 spatial layers.
Same enrichment strategy: each day token gets VP spatial CLS + daily candle
features + VP structure features before temporal attention.

v6 (1+1): 1 spatial + 1 temporal + enrichment = 24,737 params
v8 (2+1): 2 spatial + 1 temporal + enrichment = 33,281 params

── Architecture ────────────────────────────────────────────────────────────
Stage 1 (spatial, 2 layers): For each of 30 days, sample end-of-day VP
    (last hour), run 2-layer self-attention across 50 bins with CLS pooling.
    Output: (batch, 30, 32).
...
Enrichment: For each day, concat:
    ...
    Project (32 + 8 + 8 = 48) → 32 via Linear.

Stage 2 (temporal, 1 layer): CLS-pooled self-attention across 30 enriched
    day tokens. Output: (batch, 32).
...
class EnrichedTemporalV8(nn.Module):
    """Two-stage Transformer with enriched day tokens: 2 spatial + 1 temporal."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # ─── Stage 1: Spatial attention with CLS (2 layers) ───
        ...
        # ─── Stage 2: Temporal attention with CLS (1 layer) ───
        ...
        pooled = temporal_out[:, 0, :]                         # CLS token
