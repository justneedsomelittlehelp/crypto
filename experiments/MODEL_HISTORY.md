...
> **⚠ CORRECTION 2026-04-14 (same day, later in session).** The first version of this §30 reported v11-full-tb holdout at **+11.6% CAGR** with a filter-swept comparison table produced by `analyze_v11_filters.py`. That number uses label-accurate compound with no fees/slippage/latency and annualizes over the active-days-only window (~103 days for conf80). When re-run through the real backtest engine (`src/models/run_backtest_v11_tb.py`, same engine that produced v6-prime's audited +6.0% CAGR), the conf≥0.80 holdout lands at **−1.5% CAGR / −1.7% DD / 60.0% WR / 20 trades** over the full 278-day holdout. Full retraction and both-number tables live in `LABEL_REDESIGN.md` §Results. The ablation direction (full beats nopv at every filter) is preserved; the absolute "first positive holdout CAGR" claim is retracted.

**Filter-swept holdout — analyzer (idealized) vs real engine side-by-side**:
...
1. **Analyzer annualizes over 103-day active window**, engine over 278-day full holdout. Same 3% cumulative return → +11.0% vs +4.0%. **~7.6 pp of the gap.**
2. **Fees + slippage** (0.62% round-trip on ~5% barriers): per-trade EV drops from +1.02% to +0.41%. **~2-3 pp.**
3. **Engine sizing is 70% of analyzer sizing** due to `reserve_pct=0.30`. **~0.5 pp.**
4. **Fold 12 sample variance** on 20 trades clustered in the 2026 Q1 collapse fold. One extra loss shifts CAGR 5-10 pp. **Residual.**

**Ablation comparison under the real engine** (the decisive Δ still holds):
...
full beats nopv on real-engine holdout CAGR at every reasonable filter. Win-rate spread of 15-26 percentage points at every threshold — this is the cleanest real-engine evidence that VP features give the model actual discriminatory power on holdout.
...
On full holdout: sign-flip −4.3% / 50.0%, raw long −26.7% / 47.7%. The full model is not anti-aligned; it has a small positive directional bias. The conf≥0.80 filter amplifies that bias into a 60% win rate at the narrowest slice — but the resulting 20 trades are not enough to compound positive CAGR under real frictions.

### What this supports and falsifies (corrected)

**Supported (real-engine numbers):**
- **VP features carry ML-exploitable signal.** Ablation Δ is +1.5 to +8.5 pp holdout CAGR across every filter under the real engine, with win-rate spreads of 15-26 pp. Both are real and direction-stable.
- **VP features are regime-robust relative to candles.** Holdout accuracy lift (+5.0 pp) exceeds in-sample accuracy lift (+2.6 pp). Correct shape for a structural feature.
- **Confidence calibration works under clean labels.** Both models show monotonic improvement as the threshold rises, unlike v11-range.
- **Drawdown shape is materially tighter.** v11-full-tb conf≥0.80 holdout DD is 1.7% vs v6-prime honest 18.4%. Order-of-magnitude improvement. This is the strongest quantitative argument for the representation change.
- **The label redesign was the right move.** Without triple-barrier labels this ablation was literally impossible.

**Falsified:**
- *"v11 is rejected because the representation doesn't carry signal"* (2026-04-13 tentative verdict). Under clean labels the representation validates.
- *"VP features help in-sample and collapse on holdout."* Actual pattern is the opposite — VP helps more on holdout.
- **⚠ *"v11 produces the first positive holdout CAGR in project history"*** — the original §30 claim, now retracted. Real-engine holdout is −1.5% at best. The analyzer number was inflated by active-days annualization and zero frictions.

**Still open:**
- Real-engine holdout is net-negative at every filter. Model signal is real per-trade but sample count (20-30 trades over 9 months) isn't enough to compound the positive EV into a positive CAGR after fees.
- Regime change is a shared failure mode. Fold 12 (2026 Q1) is catastrophic for both models (42.2% / 35.6% acc).
- v11-full-tb does NOT beat v6-prime honest on full-period CAGR (v11 best +1.8% vs v6p +6.0% real engine). v11's win is entirely in drawdown shape and holdout discrimination.

**Verdict.** v11-full-tb is the first post-audit experiment whose ablation is unambiguously positive under real execution assumptions, and the first to cut holdout drawdown below 10%. But it does not beat v6-prime honest on compound return, and it does not produce a positive holdout CAGR. Phase 3 has a validated direction (regime-robust VP signal under clean labels) but the magnitude isn't deployment-ready without more trades or tighter regime conditioning. Artifacts preserved: `src/models/eval_v11.py`, `src/models/analyze_v11_filters.py` (with fail-loudly fix), `src/models/run_backtest_v11_tb.py`, `experiments/eval_v11_11_tb_full_results.json`, `experiments/eval_v11_11_tb_nopv_results.json`, `experiments/v11_11_tb_full_predictions.npz`, `experiments/v11_11_tb_nopv_predictions.npz`, `experiments/backtest_results_v11_tb.json`.

---

## 31. Current status (end of Stage 6) — 2026-04-14 (corrected)

- **Ablation result stands**: v11-full beats v11-nopv on every real-engine holdout filter. Δ is +1.5 to +8.5 pp CAGR and +15 to +26 pp win rate. VP features carry ML-exploitable signal — Phase 3's central hypothesis is supported for the first time in project history.
- **⚠ Absolute CAGR claim retracted**: the original §30 reported "+11.6% holdout CAGR, first positive in project history." That number came from `analyze_v11_filters.py` (no fees, active-days annualization) and does not hold under the real engine. Real-engine conf≥0.80 holdout is **−1.5% CAGR / −1.7% DD / 60.0% WR / 20 trades**. Holdout remains net-negative under realistic execution.
- **v6-prime audited honest (+6.0% full-period CAGR / −18.4% DD / holdout ≈ −5%) remains the nominal best by full-period compound return.** v11-full-tb's best real-engine full-period CAGR is +1.8% at conf≥0.80 — worse than v6-prime on full-period CAGR. v11 wins on:
  - **Holdout drawdown**: 1.7% (v11 conf80) vs 18.4% (v6p honest). Order-of-magnitude tighter.
  - **Holdout discrimination**: 60.0% win rate on clean triple-barrier labels vs ~65% under v6p's leaked labels.
  - **Ablation cleanliness**: the full-vs-nopv result is the first falsifiable VP test the project has ever run.
- **Sample count is the binding constraint for holdout CAGR.** Model signal is real per-trade (confirmed by the 60% win rate and the ablation Δ), but 20-30 trades × ~0.3% net EV per trade cannot compound into positive CAGR after fees over 278 days. More data is the only lever.
- **Prioritized next experiments (revised per `MULTI_ASSET_PLAN.md` §REFRAME)**:
  1. **⭐ Regime conditioning features** (GLD + USO + DXY + VIX + FFR level + FFR 90d change + 10Y-2Y yield curve). Directly attacks the fold 12 / regime-change problem. Single variable change on top of validated v11-full-tb baseline. Replaces the earlier "multi-asset BTC+ETH" plan which was quant-firm-shaped and wrong for this project's deployment shape.
  2. **Walk-forward rolling retraining** — retrain every 3 months on expanding windows. Other direct attack on holdout collapse. Defer until after regime features so variables stay isolated.
  3. **Diluted pyramid** (`allow_pyramiding=True` with 5% sizing) as a signal-magnitude diagnostic only. NOT deployable.
  4. **Live paper-trade the conf ≥ 0.80 wrapper** as a sanity check on drawdown shape and win-rate persistence (NOT on CAGR).
- **Option B (meta-labeling on a VP primary rule) remains demoted.** End-to-end model is already picking up VP signal; expanding the feature set is more urgent than constraining the training set.
...
## 32. Post-audit Stage 7 — prediction cadence ablation (1h on 15m data) — 2026-04-15 — ⚠ **RETRACTED: single-seed result did NOT replicate across 3 independent seeds**

**Full writeup**: `experiments/EVAL_CADENCE.md`.

### The question

v11-full-tb under Stage 6 was net-negative on real-engine holdout at every filter (best −1.5% CAGR at conf80). One hypothesis: the binding constraint is sample count. Another, raised late 2026-04-14: the 15m *prediction* cadence trains the model on microstructure noise, even though 15m *data* resolution is correct for feature computation.

These are separable. The model's 90-day input window contains every 15m bar regardless of how often anchors are placed. Striding anchors from every-bar to every-4th-bar gives ~4× fewer samples but they're far less correlated (consecutive 15m bars are near-duplicates), labels reflect moves large enough to be signal rather than noise, and deployment cadence matches a swing-trading hold.

### Implementation

Single-variable change on top of Stage 6 (v11-full-tb). Commit `e8e4681`:
- `src/models/eval_v11.py`: `STRIDE_BARS_DEFAULT=1` constant, `--stride` CLI flag, wall-clock-aligned `cadence_mask` in the eligibility block, tag auto-appends `_c{minutes}m`. `stride_bars` / `cadence_minutes` persisted to both the results JSON and the predictions NPZ.
- `src/models/run_backtest_v11_tb.py`: `--tags` / `--out` flags, auto-reads cadence from each NPZ, cadence ablation comparison block, zero-trade guard.

Training run: identical config to Stage 6 except `--stride 4` (1h cadence). Match-epochs (50), patience 15, SWA from epoch 15, 1 seed. 26.7 min on Colab A100 (~4× faster than 15m).

### Walk-forward accuracy — identical

Overall walk-forward accuracy: **1h 0.5318 vs 15m 0.5317**. The two models have identical raw classification quality. The cadence change moved *where* confidence is distributed, not *how well* the model classifies.

Per-fold highlights:
- Fold 1 (2020 bull sanity check): 0.7815 on 1h vs 0.7134 on 15m — cadence mask is correctly aligned
- Fold 6: 1h long_EV +3.70% on 15.3% long-rate vs 15m +1.23% on 49.1% long-rate — concentrated high-confidence calls
- Fold 9: 1h long_EV +3.11% on 23.0% long-rate vs 15m +1.45% on 55.3% long-rate — same pattern
- **Fold 4 mode collapse**: 1h predicts zero longs across 4,061 samples (15m was 40.6% long). Single-seed risk.
- **Fold 5 near-collapse**: 1h 77/3718 = 2.1% long rate

### Real-engine backtest — headline result

**`conf75_pause24_pyr @ 1h` (pyramid-on, conf ≥ 0.75, 24h post-SL pause)**:
- Full period: **+6.6% CAGR / −14.2% DD / 316 trades / 57.9% WR**
- Holdout: **+6.1% CAGR / −2.7% DD / 51 trades / 60.8% WR**

**Comparison vs prior bests**:

| | v6-prime honest | v11-tb 15m conf80 | **v11-tb 1h conf75-pyr** |
|---|---|---|---|
...
| Holdout n | ~25 | 20 | **51** |
| Holdout WR | ~65% (leaked) | 60.0% | **60.8%** |

**This is the first time in project history that any v11 config — or any post-audit config — has produced a positive real-engine holdout CAGR.**

v11-tb 1h matches v6-prime's nominal full-period CAGR (+6.6% vs +6.0%), **beats it on holdout CAGR by ~11 pp** (+6.1% vs ~−5%), and cuts holdout drawdown by a factor of ~7 (−2.7% vs −18.4%). It's also the first v11 config whose holdout trade count (51) is large enough to be more than a statistical punchline.

**Non-pyramid at 1h** is also positive on holdout: `conf75_pause24 @ 1h` gives +0.9% CAGR / −0.5% DD / 62.5% WR — but only 8 trades.

**Cadence ablation table** (same engine, same config, only cadence differs):
...
| conf70_pause24_pyr full | +3.5% | −55.9% | 2,568 | **+10.2%** | **−16.9%** | 422 |
| conf75_pause24_pyr full | +6.5% | −40.6% | 2,354 | +6.6% | **−14.2%** | 316 |
| conf75_pause24_pyr holdout | −18.4% | −19.3% | 164 | **+6.1%** | **−2.7%** | 51 |

1h strictly dominates 15m on nearly every config, most dramatically on drawdown (3–5× smaller) and holdout CAGR (negative → positive).

### What we can't conclude yet — three caveats

**1. Single seed.** Match-epochs, 1 seed. On 51 holdout trades at 60.8% WR the confidence interval on win rate is roughly ±13 pp. **This is the #1 reason not to deploy on this evidence.** The whole result could be one lucky draw.

**2. Confidence-scale compression (undertraining signature).** The 1h model's probability distribution on holdout is compressed toward zero — max prob 0.773, p99 0.765, zero predictions ≥ 0.80. This is consistent with match-epochs leaving the output bias less converged (4× fewer gradient updates than 15m). **Implication**: threshold comparisons are not apples-to-apples across cadences; `conf75 @ 1h` sits at approximately the same prediction quantile as `conf80 @ 15m`. Under that quantile-match framing, 1h still wins on every metric except trade count (+0.9% vs −1.5% CAGR, −0.5% vs −1.7% DD, 62.5% vs 60.0% WR, 8 vs 20 trades).

**3. Fold 4 mode collapse.** 1h model predicts zero longs on 4,061-sample fold 4 (2022 H1, crypto winter). Fold 5 nearly the same. With 1 seed and 4× less data per fold, the optimizer can land in degenerate regions for difficult regimes.

### Required follow-ups (in order)

1. **3-seed re-run** of 1h-full-tb at match-epochs. Same config, `--seeds 3`. ~80 min on Colab. Decisive-positive if conf75-pyr holdout stays > 0% CAGR and > 55% WR across all 3 seeds.
2. **Matched-gradient-steps re-run** (1h-full-tb at 200 epochs, 3 seeds). Separates "cadence is the lever" from "undertraining masked it." Only worth running if follow-up 1 passes.
3. **Envelope dynamics features** (Δwindow_hi, Δwindow_lo, Δrange_pct, bars_since_new_high/low) — orthogonal to cadence, came up in the same design discussion. The model currently sees a snapshot of the 30-day envelope but not its velocity.
4. **Regime conditioning features** per `MULTI_ASSET_PLAN.md §REFRAME` — still next-next after cadence and envelope.

### Status (original, 2026-04-15 morning)

**DIRECTION VALIDATED. MAGNITUDE NOT REPLICATED.** This is the first time Phase 3 has produced a positive real-engine holdout CAGR, and the shape of the result (small drawdown, higher win rate, concentrated high-confidence trades) is exactly what the "15m cadence was training on noise" hypothesis predicted. But it's one seed. Do not update `CLAUDE.md` build-status claims to say "deployable" or deploy a wrapper until the 3-seed replication passes.

**Artifacts**:
- Code: commit `e8e4681`, `src/models/eval_v11.py`, `src/models/run_backtest_v11_tb.py`
- Predictions: `experiments/v11_11_tb_full_c60m_predictions.npz`
- Training results: `experiments/eval_v11_11_tb_full_c60m_results.json`
- Backtest results: `experiments/backtest_results_v11_tb_cadence_ablation.json`

---

### ⚠ 2026-04-15 evening addendum — 3-seed replication retracts the holdout result

The 3-seed replication ran the same config independently at `--base-seed 42/43/44` (commit `647cc24` added `--base-seed` so runs are actually independent; the earlier `--seeds 3` path does intra-run ensembling, not replication).

**Walk-forward accuracy is stable.** Overall acc 0.5318 / 0.5224 / 0.5286 (mean 0.5276, spread 0.9pp). The cadence change doesn't damage or improve classification; same conclusion as the single-seed run.

**Full-period backtest CAGR is stable.** At `conf75_pause24_pyr`: +6.6 / +5.2 / +6.5 (mean +6.1%, spread 1.4pp). Matches the single-seed number — the full-period result is real.

**Holdout CAGR is NOT stable.** At `conf75_pause24_pyr` holdout:

| | seed42 (= original Stage 7) | seed43 | seed44 | mean |
|---|---|---|---|---|
| Holdout CAGR | +6.1% | **0.0%** | **−3.0%** | **+1.0%** |
| Holdout n_trades | 51 | **0** | **6** | 19 |
| Holdout WR | 60.8% | N/A | 50.0% | |

Seed43 produced **zero** holdout trades at conf75; seed44 produced **six**. The seed42 headline was one draw from a 9pp-spread distribution whose mean is barely positive.

**At `conf70_pause24_pyr`** (lower threshold, trade count stable at ~60 holdout trades across seeds, so the comparison is like-with-like):

| | seed42 | seed43 | seed44 | mean |
|---|---|---|---|---|
| Holdout CAGR | −10.6% | +9.8% | −6.3% | **−2.4%** (spread 20pp) |

Under the threshold that *does* produce comparable trade counts across seeds, the holdout mean CAGR is **negative**.

**What broke**: confidence calibration is unstable across seeds. Probability p50 varies 0.390 / 0.425 / 0.392 across seeds, and the tail compression at the top end varies even more. At conf75 the trade-count went 51 → 0 → 6. That's not a threshold, it's a coin flip. Any claim about "conf75 holdout CAGR" is claiming a property of something that doesn't robustly exist across training seeds.

**What's now a known model failure, not noise**: all three seeds produced `n_long=0` on fold 4 (2022 H1 bear, 4061 samples each). Fold 4 mode collapse is deterministic, not a single-seed quirk. The model saturates to zero on that regime under match-epochs training.

**Smoking gun**: seed42 was simultaneously the best walk-forward acc, the best conf75_pyr holdout CAGR, *and* the worst conf70_pyr holdout CAGR. Best-flipping-to-worst depending on which cut you take is the signature of seed noise dominating real signal.

**Retractions:**
- "First positive real-engine holdout CAGR in project history" — retracted, not reproducible
- "Beats v6-prime on holdout by ~11 pp" — retracted, that compared v6-prime against the top of a high-variance distribution
- Any fixed-threshold holdout comparison between 15m and 1h — retracted, the logit distributions shift across seeds so "conf75" isn't a fixed object

**Kept (still defensible):**
- Walk-forward acc equivalence (0.5317 vs 0.5276 mean across seeds)
- Full-period CAGR at 1h is stable and comparable to v6-prime (+5.2 to +6.6% across seeds)
- Full-period drawdown reduction at 1h is stable (−10 to −14% at 1h vs ~−40% at 15m pyramid)
- Fold 4 mode collapse is now a known architectural limit under match-epochs, not seed noise
- The methodological separation of data resolution from prediction cadence is sound; that stays

**Revised next step**: matched-gradient-steps at 1h, 3 independent seeds from the start (200 epochs, `--base-seed {42,43,44}`). The hypothesis becomes specific and testable: under-convergence produces the calibration instability, so full training should (a) raise the confidence ceiling above 0.80 and (b) stabilize conf75/80 holdout trade counts across seeds. If (a) and (b) hold, matched-steps is the cleaner comparison and we can re-examine the holdout story. If they don't, the conf-threshold framing is fundamentally fragile and we should abandon it in favor of something robust to seed noise (e.g. trade every signal above 0.5 with position-sized confidence, or per-seed threshold calibration).

**Envelope dynamics and regime features are deferred** until we have a reproducible baseline. Feature engineering on top of a seed-noise-dominated baseline is wasted effort.

**3-seed artifacts**:
- `experiments/v11_11_tb_full_c60m_seed{42,43,44}_predictions.npz`
- `experiments/eval_v11_11_tb_full_c60m_seed{42,43,44}_results.json`
- Code: commit `647cc24` (adds `--base-seed`)
- Full post-mortem: `experiments/EVAL_CADENCE.md §9`
